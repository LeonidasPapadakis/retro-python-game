...
def normalise_input(user_input):
    ...
    wordEnd = []
    start = True
    for charPos in range(0, len(no_punct)):
        if start == True:
            if no_punct[charPos] in string.ascii_lowercase:
                wordStart.append(charPos)
                start = False
                if charPos == len(no_punct)-1:
                    wordEnd.append(charPos)
                
        else:
            if no_punct[charPos] not in string.ascii_lowercase:
                wordEnd.append(charPos - 1)
                start = True
            elif charPos == len(no_punct)-1:
                wordEnd.append(charPos)

    words = []
    for pos in range(0, len(wordStart)):
        words.append(no_punct[ wordStart[pos] : wordEnd[pos] +1 ])
